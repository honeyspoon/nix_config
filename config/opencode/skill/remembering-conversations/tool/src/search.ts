...
function validateISODate(dateStr: string, paramName: string): void {
  ...
  if (isNaN(date.getTime())) {
    throw new Error(
      `Invalid ${paramName} date: "${dateStr}". Not a valid calendar date.`,
    );
  }
}
...
export async function searchConversations(
  ...
  const timeClause =
    timeFilter.length > 0 ? `AND ${timeFilter.join(" AND ")}` : "";
  ...
  return results.map((row: any) => {
    ...
    const snippet =
      exchange.userMessage.substring(0, 200) +
...
export function formatResults(
  ...
  results.forEach((result, index) => {
    const date = new Date(result.exchange.timestamp)
      .toISOString()
      .split("T")[0];
